...
def view_debt_history(hist_name):
    ...
    if debtor_id is None:
        ...
        # store a toast message in a variable to notify user that submitted name is new; therefore, debtor data and debt data must be submitted first before viewing debt history
        toast_message = "<img src='/static/fail-octagon.svg' class='icon fail-icon'><h3>NEW NAME DETECTED - SUBMIT DEBTOR'S DATA AND DEBT DATA FIRST BEFORE VIEWING DEBT HISTORY</h3>"
        ...
        if hist_data == []:
            ...
            # store a toast message in a variable to notify user that while submitted name exists, it has no debt records to view; therefore, debt data must be submitted first before viewing debt history
            toast_message = "<img src='/static/fail-octagon.svg' class='icon fail-icon'><h3>EXISTING NAME WITH NO DEBT RECORDS DETECTED - SUBMIT DEBT DATA FIRST BEFORE VIEWING DEBT HISTORY</h3>"
            ...
            hist_headers = ['ID', 'DEBT DATE', 'DEBT AMOUNT', 'INTEREST RATE (%)', 'INTEREST AMOUNT', 'TOTAL DEBT']
            ...
            # store a toast message in a variable to notify user that submitted name and their debt records exists already, and debt history table is created successfully
            toast_message = "<img src='/static/success-circle.svg' class='icon success-icon'><h3>EXISTING NAME AND THEIR DEBT RECORDS DETECTED - DEBT HISTORY TABLE CREATED SUCCESSFULLY!</h3>"
